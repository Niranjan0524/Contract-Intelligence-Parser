...
const Sidebar = () => {
  ...
  return (
    <div className={`bg-gray-800 text-white h-full transition-all duration-300 ${isCollapsed ? 'w-16' : 'w-64'}`}>
      {/* Toggle button - hidden on mobile since we have the mobile menu */}
            ...
            <li key={index}>
              <a
                ...
                className={`flex items-center px-4 py-3 text-gray-300 hover:bg-gray-700 hover:text-white transition-colors duration-200 ${
                  item.active ? 'bg-gray-700 text-white border-r-4 border-blue-500' : ''
                }`}
              >
                ...
                {!isCollapsed && (
                  <span className="ml-3 font-medium lg:inline">
                    {item.name}
                  </span>
                )}
              </a>
      ...
      {/* Bottom section with additional info when expanded */}
      {!isCollapsed && (
